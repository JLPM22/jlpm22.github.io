...
function BibTeXModal({ bibtex, onClose }) {
    ...
    return (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={onClose}>
            <div
                className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col relative border border-border"
            ...
            >
                ...
                <div className="p-5 overflow-auto flex-1">
                    <pre
                        className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm font-mono text-text whitespace-pre-wrap break-words leading-relaxed"
                        dangerouslySetInnerHTML={{ __html: formatBibtex(bibtex) }}
                    />
                </div>
                <div className="px-5 py-4 border-t border-border flex justify-end relative">
                    <button
                        ref={btnRef}
                        onClick={handleCopy}
                        className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all duration-300 ${copied
