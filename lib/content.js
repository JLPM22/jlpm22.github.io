...
export function getPapers() {
    ...
    const summariesPath = path.join(contentDirectory, 'paper_summaries.yml');
    if (fs.existsSync(summariesPath)) {
        try {
            paperSummaries = yaml.load(fs.readFileSync(summariesPath, 'utf8')) || {};
        } catch (e) {
            paperSummaries = {};
        }
    }
    ...
    const parseFile = (fileName, isJournal) => {
        ...
        parsed.data.forEach(row => {
            ...
            papers.push({
                title: row.Name,
                authors: row.Authors,
                summary: String(paperSummaries[row.Name] || '').trim(),
...
export function getAwards() {
    ...
    const fileContents = fs.readFileSync(fullPath, 'utf8');
    const parsed = Papa.parse(fileContents, { header: true, skipEmptyLines: true });
    const buttonConfigPath = path.join(contentDirectory, 'award_buttons.yml');
    let buttonConfig = {};
    if (fs.existsSync(buttonConfigPath)) {
        try {
            buttonConfig = yaml.load(fs.readFileSync(buttonConfigPath, 'utf8')) || {};
        } catch (e) {
            buttonConfig = {};
        }
    }
    ...
    return parsed.data.map(row => {
        ...
        const awardName = (row.Name || '').trim();
        const awardButtons = buttonConfig[awardName] || {};
        ...
        const relatedLinks = Object.entries(row)
            ...
            .map(([key, value]) => {
                ...
                return separatorIndex > -1
                    ...
                    : { label: (awardButtons.links?.[key] || '').trim(), url: raw };
            });
        ...
        return {
            ...
            certificateLabel: (awardButtons.certificate || 'Certificate').trim(),
            relatedLinks,
        };
